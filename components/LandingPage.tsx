...
export default function LandingPage() {
  ...
  const { trackEvent, trackPlanSelection, trackSignUp, trackLogin } = useGoogleAnalytics();
  ...
  // Processar retorno do login com Google
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const googleSignin = urlParams.get('google_signin');
    
    if (googleSignin === 'true' && session?.user) {
      console.log('🔄 Processando retorno do login com Google...');
      
      // Recuperar dados salvos do localStorage
      const savedPlan = localStorage.getItem('landingPageSelectedPlan');
      const savedCampaignData = localStorage.getItem('landingPageCampaignData');
      const savedSource = localStorage.getItem('landingPageSource');
      
      if (savedPlan && savedSource === 'google_oauth') {
        const processGoogleSignIn = async () => {
          try {
            const plan = JSON.parse(savedPlan);
            const campaignInfo = savedCampaignData ? JSON.parse(savedCampaignData) : null;
            
            console.log('📊 Dados recuperados:', { plan, campaignInfo });
            
            // Verificar status do usuário via API
            const response = await fetch('/api/landing-page/google-signin', {
              method: 'POST',
              headers: {
            ...
            // Se o usuário já tem premium, redirecionar
            if (data.user.premium) {
              console.log('✅ Usuário já é premium, redirecionando...');
              router.push('/');
              return;
            }
            
            // Se não é premium, mostrar opções de pagamento
            ...
            localStorage.removeItem('landingPageSelectedPlan');
            localStorage.removeItem('landingPageCampaignData');
            localStorage.removeItem('landingPageSource');
  ...
  const handlePlanSelect = (plan: Plan) => {
    ...
    trackPlanSelection(plan.id, plan.title, plan.price);
    trackEvent('plan_selected', 'subscription', plan.title, plan.price);
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Rastrear tentativa de login com Google
      trackEvent('google_signin_attempt', 'authentication', 'landing_page');
      
      // Salvar dados do plano e campanha no localStorage para usar após o login
      ...
      localStorage.setItem('landingPageSelectedPlan', JSON.stringify(selectedPlan));
      localStorage.setItem('landingPageCampaignData', JSON.stringify(campaignInfo));
      localStorage.setItem('landingPageSource', 'google_oauth');

      // Fazer login com Google
      const result = await signIn('google', {
        callbackUrl: '/c?google_signin=true',
        redirect: false,
      });
      ...
      // Rastrear sucesso do login com Google
      trackLogin('google');
      trackEvent('google_signin_success', 'authentication', 'landing_page');

      // Se o login foi bem-sucedido, o usuário será redirecionado
      // e o useEffect processará os dados salvos
    } catch (error) {
      console.error('Erro no login com Google:', error);
      setError('Erro ao fazer login com Google. Tente novamente.');
      
      // Rastrear erro no login com Google
      trackEvent('google_signin_error', 'authentication', 'landing_page');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handlePaymentMethodSelect = (method: 'pix' | 'card') => {
    ...
    // Rastrear seleção de método de pagamento
    trackEvent('payment_method_selected', 'payment', method, selectedPlan?.price);
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-center">
                      <button
                        ...
                        disabled={isLoading}
                        className="w-full bg-white text-gray-900 font-bold py-3 rounded-lg hover:bg-gray-100 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 border border-gray-300"
                      >
